...
exports.YieldController = void 0;
...
class YieldController {
     ...
     */
    static activateYield(dataSource) {
        ...
        `, [paymentId, userEmail]);
                ...
                const existingYield = await queryRunner.query(`SELECT * FROM yield_activations WHERE payment_id = $1 AND status = 'active'`, [paymentId]);
                if (existingYield.length > 0) {
                    await queryRunner.rollbackTransaction();
                    res.status(400).json({ success: false, error: 'Yield is already active for this payment.' });
                    return;
                }
                const etherFuseCustomerQuery = await queryRunner.query(`SELECT * FROM etherfuse_customers WHERE user_id = $1`, [payment.user_id]);
                ...
                if (!etherFuseCustomer || etherFuseCustomer.kyc_status !== 'approved') {
                    ...
                    const kustodiaBankAccountId = (0, uuid_1.v4)();
                    // Cast to any to bypass TS error for JS service
                    const onboardingResult = await etherFuse.generateOnboardingUrl(payment.user_id, kustodiaBankAccountId);
     ...
     * @returns An Express request handler.
     */
    static getYieldStatus(dataSource) {
        return async (req, res) => {
            try {
                const { id: paymentId } = req.params;
        ...
        `, [paymentId, userEmail]);
                ...
                const daysSinceActivation = Math.floor((new Date().getTime() - new Date(yieldData.activated_at).getTime()) / (1000 * 60 * 60 * 24));
     ...
     * @returns An Express request handler.
     */
    static getYieldHistory(dataSource) {
        return async (req, res) => {
            try {
                const { id: paymentId } = req.params;
        ...
        `, [paymentId, userEmail, limit]);
                const history = historyQuery.map((row) => ({
     ...
     * @returns An Express request handler.
     */
    static handleWebhook(dataSource) {
        return async (req, res) => {
            try {
     ...
     */
    static async handleEarningsUpdate(dataSource, data) {
        const queryRunner = dataSource.createQueryRunner();
        await queryRunner.connect();
        await queryRunner.startTransaction();
        try {
     ...
     */
    static async handleOrderCompletion(dataSource, data) {
     ...
     * Handle KYC approval
     */
    static async handleKycApproval(dataSource, data) {
        try {
...
exports.YieldController = YieldController;
