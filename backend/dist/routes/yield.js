...
exports.createYieldRoutes = void 0;
...
const authenticateUser = (req, res, next) => {
    ...
    const userEmail = req.headers['x-user-email']; // Temporary for testing
    if (!userEmail) {
        res.status(401).json({
            success: false,
...
const createYieldRoutes = (dataSource) => {
    const router = express_1.default.Router();
    /**
     * Yield Generation Routes
     */
    // Activate yield generation for a payment
    router.post('/payments/:id/activate-yield', authenticateUser, yieldController_1.YieldController.activateYield(dataSource));
    // Get current yield status for a payment
    router.get('/payments/:id/yield-status', authenticateUser, yieldController_1.YieldController.getYieldStatus(dataSource));
    // Get yield earnings history for a payment
    router.get('/payments/:id/yield-history', authenticateUser, yieldController_1.YieldController.getYieldHistory(dataSource));
    // EtherFuse webhook endpoint (no auth required - signature verified in controller)
    router.post('/etherfuse/webhook', yieldController_1.YieldController.handleWebhook(dataSource));
    /**
     * Admin/Internal Routes (require admin authentication)
     */
    // Manual yield calculation trigger (admin only)
    router.post('/admin/calculate-yields', (req, res) => {
    ...
    router.get('/admin/etherfuse/health', async (req, res) => {
        try {
    ...
    router.get('/admin/cetes-rate', async (req, res) => {
        try {
...
exports.createYieldRoutes = createYieldRoutes;
