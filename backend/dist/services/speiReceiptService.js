...
exports.SPEIReceiptService = void 0;
...
class SPEIReceiptService {
     ...
     */
    static generateReferenceNumber(receiptData) {
     ...
     */
    static getBankNameFromClabe(clabe) {
        if (!clabe || clabe.length < 3)
     ...
     */
    static generateSecurityElements(receiptData) {
     ...
     */
    static validateReceiptSecurity(receiptData) {
     ...
     */
    static generateQRCodeData(receiptData) {
        const qrData = {
     ...
     */
    static async generateReceiptFromPayment(payment, user, junoTransactionId, options = {}) {
        // Fetch Juno transaction details if available
        let junoTransaction = null;
        if (junoTransactionId) {
            try {
                junoTransaction = await (0, junoService_1.getWithdrawalStatus)(junoTransactionId);
            }
            catch (error) {
     ...
     */
    static async generateReceipt(data, options = {}) {
     ...
     */
    static generateReceiptHTML(data, options) {
...
<body>
     ...
     */
    static async saveReceiptToFile(data, options = {}) {
     ...
     */
    static cleanupOldReceipts(maxAgeHours = 24) {
        if (!fs.existsSync(this.TEMP_DIR))
            return;
...
exports.SPEIReceiptService = SPEIReceiptService;
SPEIReceiptService.RECEIPT_TEMPLATE_PATH = path.join(__dirname, '../templates/spei-receipt.html');
SPEIReceiptService.TEMP_DIR = path.join(__dirname, '../../temp/receipts');
// Mexican Banking Security Constants (CEP Compliance)
// Nvio is the actual bank emisor for SPEI transfers
SPEIReceiptService.NVIO_INSTITUTION_CODE = 'NVIO001';
SPEIReceiptService.NVIO_BANK_NAME = 'Nvio Pagos México';
SPEIReceiptService.KUSTODIA_SERVICE_NAME = 'Kustodia';
SPEIReceiptService.SECURITY_ALGORITHM = 'sha256';
SPEIReceiptService.DIGITAL_SIGNATURE_KEY = process.env.SPEI_SIGNATURE_KEY || 'nvio-spei-2024-secure-key';
// SPEI/CEP Constants
SPEIReceiptService.SPEI_CERTIFICATE_SERIAL_PREFIX = 'NVI';
SPEIReceiptService.CEP_VERSION = '1.0';
