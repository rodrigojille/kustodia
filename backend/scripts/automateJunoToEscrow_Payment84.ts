...
// Import custom escrow funding logic for Payment 84
import { main as fundEscrowPayment84 } from "./sendEscrowFunds_Payment84";

const JUNO_ENV = process.env.JUNO_ENV || 'stage';
const JUNO_API_KEY = JUNO_ENV === 'stage' ? process.env.JUNO_STAGE_API_KEY! : process.env.JUNO_API_SECRET!;
...
const MXNB_AMOUNT = 5000.0; // Amount to withdraw for Payment 84
...
async function withdrawFromJuno() {
  ...
  // Generate unique idempotency key for this Payment 84 withdrawal
  const CLI_NONCE = process.argv[2];
  const ENV_NONCE = process.env.JUNO_NONCE;
  const NONCE = CLI_NONCE || ENV_NONCE || Date.now().toString();
  const idempotencyKey = process.env.JUNO_IDEMPOTENCY_KEY || `kustodia-payment84-${NONCE}`;
...
async function main() {
  console.log(`\n🎯 Starting automated flow for Payment 84`);
  console.log(`Target: Transfer 5000 MXNB from Juno → Bridge Wallet → Escrow Smart Contract`);
  ...
  try {
    ...
    // Step 3: Create and fund escrow on smart contract (Payment 84 / Escrow 72)
    console.log(`\n🔒 Step 3: Creating escrow on smart contract...`);
    await fundEscrowPayment84();

    console.log(`\n🎉 SUCCESS! Complete flow executed for Payment 84:`);
    ...
    console.log(`✅ Payment 84 is now ACTIVE with on-chain custody`);

  } catch (error) {
    console.error(`\n❌ Error in automated flow for Payment 84:`, error);
    throw error;
  }
}
