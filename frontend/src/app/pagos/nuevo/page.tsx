...
export default function NuevoMovimientoPage() {
  ...
  // Handler para 'Cobrar' (requestPayment)
  async function handleCobrarSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    setLoading(true);
    setError(null);
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-10">
      <div className="max-w-xl mx-auto bg-white rounded-xl shadow border border-gray-200 p-8 mt-8">
        <h1 className="text-2xl font-bold text-black mb-6">Nuevo movimiento</h1>
        <div className="flex gap-2 mb-8">
          <button
            className={`px-4 py-2 rounded-t transition font-semibold border-b-2 ${activeTab === 'cobrar' ? 'border-blue-600 text-blue-700 bg-blue-50' : 'border-transparent text-gray-500 bg-gray-50'}`}
          ...
          <button
            className={`px-4 py-2 rounded-t transition font-semibold border-b-2 ${activeTab === 'pagar' ? 'border-blue-600 text-blue-700 bg-blue-50' : 'border-transparent text-gray-500 bg-gray-50'}`}
        ...
        {activeTab === 'cobrar' ? (
          <form className="space-y-4 animate-fade-in" onSubmit={handleCobrarSubmit}>
            <div>
              <label className="block text-sm font-medium text-black mb-1">Correo del pagador *</label>
              <input type="email" name="payer_email" className="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-600 text-black bg-white placeholder-gray-400" placeholder="usuario@ejemplo.com" required />
            </div>
            <div>
            ...
            <div>
              ...
              <select name="currency" className="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-600 text-black bg-white placeholder-gray-400">
                <option value="MXN">MXN (Peso Mexicano)</option>
                <option value="USD">USD (Dólar)</option>
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-black mb-1">Concepto o referencia *</label>
            ...
            {showCommission && (
              <div className="bg-blue-100 rounded p-4 space-y-3 animate-fade-in">
                <div>
                  ...
                  <input type="number" name="commission_percent" min="0" max="100" step="0.01" className="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-600 text-black bg-white placeholder-gray-400" placeholder="Ej: 2.5" required={showCommission} />
                </div>
                <div>
            ...
            <div className="flex justify-end gap-2 mt-6">
              ...
              <button type="submit" className="px-4 py-2 rounded bg-blue-600 text-white font-semibold hover:bg-blue-700 transition">Crear pago</button>
            </div>
          </form>
        ) : (
          <form className="space-y-4 animate-fade-in" onSubmit={handlePagarSubmit}>
            ...
            <div className="flex justify-end gap-2 mt-6">
              <Link href="/pagos" className="px-4 py-2 rounded bg-gray-100 text-black font-semibold hover:bg-gray-200">Cancelar</Link>
              <button type="submit" className="px-4 py-2 rounded bg-blue-600 text-white font-semibold hover:bg-blue-700 transition">Iniciar pago</button>
            </div>
          </form>
        )}
      </div>
